...
def rewards_table_create(n_rows, n_cols):
    # Now let's initialize the rewards table (everything at -1), using the "minimizing punishment strategy"
    rewards_table = -np.ones((n_rows,n_cols))
...
def is_terminal_state(current_state, rewards_table):
    ...
    # check if the reward is -1, then is not a terminal state
    if (rewards_table[row_index][column_index] == -1):
        return False
    else:
...
def main():
    ...
    rewards_table = rewards_table_create(environment_rows, environment_columns)
    # Terminal states, goal and whole!
